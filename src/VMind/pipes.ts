...
export const wordCloudData = (spec: any, context: Context) => {
  ...
  spec.data = {
    ...
    values: dataset
      .filter(
        (d: any) =>
          d[color!] && d[size!] && d[color!].length > 0 && d[size!].length > 0
      )
...
export const cartesianLine = (spec: any, context: Context) => {
  ...
  if (cell.color) {
    ...
    const remainedFields = dataFields.filter(
      (f) => !spec.xField.includes(f) && spec.yField !== f
    );
