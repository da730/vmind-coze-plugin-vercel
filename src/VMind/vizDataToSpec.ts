...
export const patchChartTypeAndCell = (
  ...
  if (chartType === "WORD CLOUD") {
    ...
    if (!cellNew.size || !cellNew.color || cellNew.color === cellNew.size) {
      ...
      if (!cellNew.color) {
        const newColor =
          cellNew.text ?? cellNew.word ?? cellNew.label ?? cellNew.x;
        if (newColor) {
  ...
  if (chartType === "DYNAMIC BAR CHART") {
    ...
    if (!cell.time || cell.time === "" || cell.time.length === 0) {
      ...
      const usedFields = Object.values(cellNew).filter(
        (f) => !Array.isArray(f)
      );
  ...
  //直角坐标图表 必须有x字段
  if (
    CARTESIAN_CHART_LIST.map((chart) => chart.toUpperCase()).includes(chartType)
  ) {
    const cellNew = { ...cell };
    if (!cellNew.x) {
      const usedFields = Object.values(cell);
...
export const checkChartTypeAndCell = (
  chartType: string,
  cell: any
): boolean => {
  switch (chartType) {
